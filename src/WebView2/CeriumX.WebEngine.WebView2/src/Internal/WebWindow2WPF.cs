...
//=========================================================================
// 文件名称：WebWindow2WPF.cs
// 项目名称：Web引擎服务产品中间件
// 创建时间：2022-10-13 16:06:23
// 创建人员：宋杰军
...
internal sealed class WebWindow2WPF<TControlType> : IWebWindow<TControlType>
        where TControlType : class
{
    ...
    public WebWindow2WPF(WebContext context, CoreWebView2Environment? environment, WebOptions options)
    {
        ...
        _webView.Initialized += WebView2_Initialized;

        BrowserControl = _webView as TControlType;

        _context.Logger.Info($"成功创建适用于 WPF 的浏览器窗口。");
    }
    ...
    private async void WebView2_Initialized(object? sender, EventArgs e)
    {
        ...
        // 调用 EnsureCoreWebView2Async 方法显式初始化底层，此处须保持上下同一线程。
        ...
        // 是否使用新窗口来打开弹出窗口
        if (_options.UseNewWindowToOpenPopup is false)
        {
